    ...
    <div>
      ...
      <div className="space-y-3">
        {athlete.recentWorkouts.map((workout) => (
          <div
            key={workout.id}
            className="bg-black-elevated border border-gray-800 rounded-lg p-4 flex items-center gap-4 hover:border-gold-primary/50 transition-all duration-200"
          >
            {/* Verification status */}
            {workout.verified ? (
              <CheckCircle2 className="text-gold-primary shrink-0" size={22} />
            ) : (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <p className="text-white font-semibold truncate">{workout.type}</p>
                {workout.verified && (
              ...
              <p className="text-gray-500 text-sm">{daysAgo(workout.date)}</p>
            </div>

            {/* Duration */}
            <div className="flex items-center gap-1.5 text-gray-400 shrink-0">
              <Clock size={14} />
              <span className="text-sm font-mono">{formatDuration(workout.duration)}</span>
            </div>
